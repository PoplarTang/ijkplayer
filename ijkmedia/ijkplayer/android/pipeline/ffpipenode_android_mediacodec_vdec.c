...
static int reconfigureMediaCodec(JNIEnv *env, IJKFF_Pipenode *node)
{
    ...
    SDL_JNI_DeleteLocalRefP(env, &opaque->jsurface);
...
static int feedInputBuffer(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    ...
    if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
        ...
        ALOGE("bsfc->filter(%d): %p[%d] -> %p[%d]", d->bfsc_ret, d->pkt.data, (int)d->pkt.size, d->pkt_temp.data, (int)d->pkt_temp.size);
#else
        ALOGE("raw [%d][%d] %02x%02x%02x%02x%02x%02x%02x%02x", (int)d->pkt_temp.size,
            (int)opaque->nal_size,
            d->pkt_temp.data[0],
            d->pkt_temp.data[1],
            d->pkt_temp.data[2],
        ...
        ALOGE("input[%d][%d][%lld,%lld (%d, %d) -> %lld] %02x%02x%02x%02x%02x%02x%02x%02x", (int)d->pkt_temp.size,
            (int)opaque->nal_size,
...
static int drainOutputBuffer(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *dequeue_count)
{
    ...
    if (output_buffer_index == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
        ALOGE("AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED\n");
        // continue;
    } else if (output_buffer_index == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
        // let SDL_AMediaFormat itself handle this.
        ...
        ALOGE("%s: render buffer %d\n", __func__, output_buffer_index);
        amc_ret = SDL_AMediaCodec_releaseOutputBuffer(opaque->acodec, output_buffer_index, true);
        if (amc_ret != SDL_AMEDIA_OK) {
